...
def crosscheck_videos(video_path, ann_file):
    ...
    for idx, vid in enumerate(existing_vids):
        ...
        if len(basename) == 13:
            ...
            raise RuntimeError(f"Unknown filename format: {vid}")

    # Read and get video IDs from annotation file
...
def main(video_path, ann_file, output_filename):
    ...
    cmd_base = "yt-dlp -f best -f mp4 --ignore-errors "
    ...
    with open(output_filename, "w") as fobj:
        for vid in non_existing_videos:
            ...
            fobj.write("%s\n" % cmd)
    print(f"Generated download commands for {len(non_existing_videos)} videos in {output_filename}")
